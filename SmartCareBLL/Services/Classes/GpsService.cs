...
namespace SmartCareBLL.Services.Classes
{
    public class GpsService : IGpsService
    {
        ...
        public async Task<GpsDTO> GetGpsLocationAsync(int userId)
        {
            if (userId <= 0)
            {
                throw new ArgumentException("Invalid user ID.");
            }
            var gpsLocations = await _unitOfWork.GetRepository<GpsLocation>().GetAllAsync();
        ...
        public async Task UpdateGpsLocationAsync(string deviceIdentifier, GpsUpdateDTO gpsUpdateDTO)
        {
            ...
            if (gpsLocation == null)
            {
                ...
                await _unitOfWork.GetRepository<GpsLocation>().AddAsync(gpsLocation);
            }
            else
            {
                gpsLocation.Latitude = gpsUpdateDTO.Latitude;
                gpsLocation.Longitude = gpsUpdateDTO.Longitude;
                _unitOfWork.GetRepository<GpsLocation>().Update(gpsLocation);
            }
